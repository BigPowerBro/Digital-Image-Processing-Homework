...
from FCN_network import VGGNet
from torch.optim.lr_scheduler import StepLR
from UNET import UNet
from tiny_fcn import TinyFCN
...
def train_one_epoch(model, dataloader, optimizer, criterion, device, epoch, num_epochs):
    ...
    for i, (image_rgb, image_semantic) in enumerate(dataloader):
        ...
        if epoch % 5 == 0 and i == 0:
            save_images(image_rgb, image_semantic, outputs, 'D:\\study\\DIP\\hw2\\Pix2Pix\\result\\edges2shoes\\train_results', epoch)
...
def validate(model, dataloader, criterion, device, epoch, num_epochs):
    ...
    with torch.no_grad():
        for i, (image_rgb, image_semantic) in enumerate(dataloader):
            ...
            # Save sample images every 5 epochs
            if epoch % 5 == 0 and i == 0:
                save_images(image_rgb, image_semantic, outputs, 'D:\\study\\DIP\\hw2\\Pix2Pix\\result\\edges2shoes\\val_results', epoch)
...
def main():
    ...
    train_dataset = FacadesDataset(list_file='D:\\study\\DIP\\hw2\\Pix2Pix\\train_list.txt')
    val_dataset = FacadesDataset(list_file='D:\\study\\DIP\\hw2\\Pix2Pix\\val_list.txt')
    ...
    vgg = VGGNet(False, "vgg11")
    model = FullyConvNetwork(vgg,3).to(device)
    criterion = nn.L1Loss()
