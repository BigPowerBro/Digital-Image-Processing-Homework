...
def cal_slope(p1,p2):
    ...
    if p1[0]==p2[0]:
        ...
        d = (p1[1]*p2[0]-p2[1]*p1[0])/(p2[0]-p1[0])
    return alpha,d
...
def create_mask_from_points(points, img_h, img_w):
    ...
    for i in range(num_points):
        ...
        alpha1,_ = cal_slope(p_back,p)
        alpha2,_ = cal_slope(p,p_next)
        if alpha1 != alpha2 and (p != p_next).all():
            points_after_filter.append(p)
    num_points = np.size(points_after_filter,0)

    ## 2.获取边界点集合（不包含顶点）
    scan_border_dict = {} # 记录每个纵坐标 有哪些边界点 值为边界点的横坐标
    for i in range(num_points):
        p = points_after_filter[i]
        p_back = points_after_filter[i-1]
        alpha,d = cal_slope(p_back,p)
        y_min = np.min([p[1],p_back[1]])
        y_max = np.max([p[1],p_back[1]])
        if d is not None:
            if np.isclose(alpha,0):  # 若水平 则跳过
                continue
            k = np.tan(alpha)
            for y in range(y_min+1,y_max):
                if y in scan_border_dict:
                    scan_border_dict[y].append((y-d)/k)
                else:
                    scan_border_dict[y] = [(y-d)/k]
        else:
            x = p[0]
            for y in range(y_min+1,y_max):
                if y in scan_border_dict:
                    scan_border_dict[y].append(x)
                else:
                    scan_border_dict[y] = [x]
    ...
    for i in range(num_points):
        ...
        y = p[1]
        p_back = points_after_filter[i-1]
        p_next = points_after_filter[(i+1)%num_points]
        alpha1,_ = cal_slope(p_back,p)
        if np.isclose(alpha1,0):
            continue
        else:
            alpha2,_ = cal_slope(p,p_next)
            if np.isclose(alpha2,0):
                p_next = points_after_filter[(i+2)%num_points]
            if (y-p_back[1])*(y-p_next[1]) < 0:
                if y in scan_border_dict:
                    scan_border_dict[y].append(p[0])
                else:
                    scan_border_dict[y] = [p[0]]
    ...
    for r in scan_border_dict.keys():
        ...
        for i in range(nps):
            c_start = c_list[2*i]
            c_end = c_list[2*i+1]
            mask[r,int(np.ceil(c_start)):int(c_end)+1] = 255
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    kernel = torch.tensor([[0, 1, 0],
                           [1, -4, 1],
                           [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0).expand(-1, 3, -1, -1).to(device) #卷积核
    
    lap_fore = torch.nn.functional.conv2d(foreground_img, kernel, padding=1)
    lap_blend = torch.nn.functional.conv2d(blended_img, kernel, padding=1)

    return torch.nn.functional.mse_loss(lap_fore[foreground_mask.bool()],lap_blend[background_mask.bool()])
